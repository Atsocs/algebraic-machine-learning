...
def sparse_crossing(a, b):
        ...
        while True:
            ...
            if (not Del) or (not same(Del_prime, Del)):
                ...
                if closure:
                    ...
                    dual = nx.algorithms.transitive_closure(dual)
                # ---- COLORING ------
                nx.set_node_attributes(master, {psi: 'pink'}, "color")
                nx.set_node_attributes(dual, {dual_of(psi): 'pink'}, "color")
                # --------------------
    ...
    for eps in U:
        ...
        if closure:
            ...
            dual = nx.algorithms.transitive_closure(dual)
        # ---- COLORING ------
        nx.set_node_attributes(master, {eps_prime: 'pink'}, "color")
        nx.set_node_attributes(dual, {dual_of(eps_prime): 'pink'}, "color")
        # --------------------
...
if __name__ == '__main__':
    ...
    sparse_crossing(target, term(T1_pos))
    # enforce_positive_trace_constraints()
    # enforce_negative_trace_constraints()
    # enforce_positive_trace_constraints(True, term(T2_pos))
    # sparse_crossing(target, term(T2_pos))
    # enforce_negative_trace_constraints()
    # enforce_positive_trace_constraints(True, term(T2_pos))
    ...
    if drawing:
        ...
        pyplot.subplot(ax1)
        draw_colored_graph(master, spring=False)
        pyplot.subplot(ax2)
        draw_colored_graph(dual, spring=False)
        pyplot.show()
    ...
    print("(target < T_i_pos) =", [less("master", target, term(x)) for x in pos_def])  # Should be a list of True's

    print(pformat(list(master.nodes)))
    print(pformat(list(dual.nodes)))
    # print_edges = True
    print_edges = False
    if print_edges:
        print("master")
